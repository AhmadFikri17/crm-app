...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        ...
        if (empty($customer_id) || empty($interaction_type)) {
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO interactions (customer_id, user_id, interaction_type, notes, interaction_date) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$customer_id, $user_id, $interaction_type, $notes, $interaction_date]);
                $success = 'Interaksi berhasil ditambahkan!';
                // Reset form
                $customer_id = 0;
            } catch(PDOException $e) {
                $error = 'Gagal menambahkan interaksi';
            }
        }
    }
}
...
<head>
    ...
    <title>Tambah Interaksi - CRM System</title>
            ...
            <header class="bg-white border-b border-[#D1FAE5] px-6 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        ...
                        <h1 class="text-xl font-semibold text-[#1F2937]">Tambah Interaksi</h1>
                    </div>
                    <div class="flex items-center space-x-4">
                        <div class="flex items-center space-x-3">
                        ...
                        <form method="POST" action="" id="interactionForm" class="space-y-5">
                            ...
                            <div>
                                ...
                                <textarea name="notes" rows="4"
                                          class="w-full px-4 py-2 border border-[#D1FAE5] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#22C55E]"
                                          placeholder="Masukkan catatan interaksi"><?php echo isset($_POST['notes']) ? htmlspecialchars($_POST['notes']) : ''; ?></textarea>
                            </div>
